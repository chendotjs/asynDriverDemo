 ...
 * The `parm string` will bind to `param value` (which is mapped to device
 * property
 */
...
public:
  ...
  //		std::unique_ptr<struct Params> params = std::unique_ptr<struct
  //Params>(new struct Params);
  static const size_t
      NUM_PARAMS; // need `static` to be initialized before constructor
...
protected:
  template <typename T> using handler_t = std::function<int(asynUser *, T *)>;

  template <typename T> using handlerMap_t = std::map<int, handler_t<T>>;
