...
const size_t DummyDriver::NUM_PARAMS =
    sizeof(struct DummyDriver::Params) / sizeof(int);
static const char driverName[] = "DummyDriver";

DummyDriver::DummyDriver(const char *portName)
    : asynPortDriver(
...
void DummyDriver::registerHandler_Int32(int func_index,
                                        DummyDriverFuncPtr<epicsInt32> func) {
...
asynStatus DummyDriver::writeInt32(asynUser *pasynUser, epicsInt32 value) {
  ...
  std::cout << "writeInt32 called, value: " << value
            << ", function: " << function << std::endl;

  int feedback = 0;
  try {
    auto handler = handlerMap_Int.at(function);
    feedback = handler(pasynUser, &value); // actually call echoHandler
...
void DummyDriverRegister(void) { iocshRegister(&initFuncDef, initCallFunc); }
